  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      ...
      <div className="z-10 max-w-5xl w-full items-center justify-between font-mono text-sm lg:flex">
        ...
        <p className={styles.p_styled}>{!addressWallet ? (

          <ul> <li><Link href="/dashboard"> Back  </Link></li>
          ...
          <ul>
            <li><Link href="/dashboard"> Back  </Link></li>
            <li>Address : {addressWallet} </li>
            <li>ChainID : {chain?.id}</li>
            <li> <Link href={chain?.blockExplorers?.default.url + "/address/" + addressWallet} target="_blank">Explorer : {chainName}</Link>  </li>
          </ul>
        )}
        </p>
        ...
        <div className={styles_grid['grid-container']}>
          ...
          {assetArray && assetArray.map((item, index) => (
            <div key={index} className={styles_grid['grid-item']}>
              <div>Name: {item.name}</div>
              <div>Balance: {ethers.utils.formatUnits(item.balance, Number(item.decimals))}</div>
