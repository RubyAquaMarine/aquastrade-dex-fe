...
import { Exchange } from "./DexRuby";
import { WalletButton } from "./Amounts";

import { usePoolsWithWETHAddress } from "../Utils/usePool";


import {ROUTER_RUBYSWAP} from "../Utils/config";

const ROUTER_ADDRESS = ROUTER_RUBYSWAP;
const SwapRuby = () => {
  const { account, error, chainId, activateBrowserWallet } = useEthers();
  const [poolsLoading, pools] = usePoolsWithWETHAddress(ROUTER_ADDRESS, undefined);//todo
  if (error) {
    ...
    <>
      <div>
        ...
        {chainId !== desiredChainId && (
          <div>
            <p>
              You are currently on chain ID {chainId}. To use this app, please switch to chain ID {desiredChainId}.
            </p>
        ...
        {chainId === desiredChainId && (
          <div>
            ...
            <div> <Stack justifyContent="right" direction="row" padding="10px">
              <WalletButton />
            </Stack></div>
            ...
            <div> <Stack justifyContent={"flex-end"} width={"100%"} direction={"row"} sx={{ position: "absolute", bottom: 0 }}>
