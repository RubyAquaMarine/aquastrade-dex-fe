...
import abiRouter from "../Utils/router.json";
...
import { AmountIn, AmountOut, CurrencySelector } from "./Amounts";
...
const getBalance = (token, tokenBalance) => formatUnits(tokenBalance ?? parseUnits("0"));
...
export function Exchange(props) {
  ...
  const routerContract = new Contract(ROUTER_ADDRESS, abiRouter);
  ...
  const formValueIsGreaterThan0 = fromValueBigNumber.gt(parseUnits("0.000000000001"));
  ...
  const { state: swapExecuteState, send: swapExecuteSend } = useContractFunction(routerContract, swapFunction, {
  ...
  const onSwapRequested = () => {
    switch (swapFunction) {
      case "swapExactETHForTokens":
        swapExecuteSend(0, [fromToken, toToken], account, Math.floor(Date.now() / 1000) + 60 * 20, { value: fromValueBigNumber }).then(
          (_) => {
            setFromValue("0");
          }
        );
    ...
    <div className={"exchange"}>
      <Box className={"rounded filled background"}>
        <Stack spacing={2} direction="column" justifyContent="center">
          ...
          <FormGroup row={true} className={"rounded filled group centerContent"}>
            <AmountOut fromToken={fromToken} toToken={toToken} amountIn={fromValueBigNumber} pairContract={pairAddress} />
            <CurrencySelector value={toToken} onSelect={onToTokenChange} currencies={counterpartTokens} showEmpty={"Select currency"} />
          </FormGroup>
          <FormHelperText id="outlined-weight-helper-text">
        ...
        <Stack alignContent="center" justifyContent="center" spacing={2} direction="row" sx={{ paddingTop: "1em" }}>
          {approvedNeeded && !isSwapping ? (
            <Button variant="contained" disabled={!canApprove} onClick={onApproveRequested}>
              {isApproving ? "Approving..." : "Approve"}
            </Button>
          ) : (
            <Button variant="contained" disabled={!canSwap} onClick={onSwapRequested}>
